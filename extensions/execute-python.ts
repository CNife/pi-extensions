...
import { defineTool, highlightCode, type ExtensionAPI } from "@earendil-works/pi-coding-agent";
...
const executePythonTool = defineTool({
	...
	parameters: Type.Object({
		...
		packages: Type.Optional(
			Type.Array(Type.String(), {
				description:
					"PyPI dependencies, e.g. 'pyyaml' or 'pyyaml>=1.0'",
				default: [],
			}),
		),
		python_version: Type.Optional(
			Type.String({
				description:
					"Python version, e.g. '3.12', passed to uv --python",
			}),
		),
		...
		try {
			...
			const result = await new Promise<ExecutePythonResult>((resolve) => {
				...
				const scheduleUpdate = () => {
					...
					updateDirty = true;
					const delay =
						UPDATE_THROTTLE_MS - (Date.now() - lastUpdateAt);
					if (delay <= 0) {
				...
				child.on("close", (code) => {
					...
					const finalStderr = timedOut
						? (stderr ? `${stderr}\n[timeout after ${timeout}s]` : `timeout after ${timeout}s`)
						: stderr;
	...
	renderResult(result, { expanded, isPartial }, theme, context) {
		...
		if (state.startedAt) {
			...
			statusParts.push(theme.fg("muted", `${label} ${formatDuration(endTime - state.startedAt)}`));
		}
