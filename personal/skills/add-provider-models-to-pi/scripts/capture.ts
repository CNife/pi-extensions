...
// ---- message_end 消息体（pi 未导出类型，按使用字段定义）----
...
function summarizeContent(content: ContentBlock[]): {
  textBlocks: number;
  textChars: number;
  ...
  const thinkingBlocks = content.filter((b) => b.type === "thinking") as Array<{
    thinking: string;
  }>;
  const toolCalls = content.filter((b) => b.type === "toolCall") as Array<{
    name: string;
    arguments: Record<string, unknown>;
  }>;
  return {
    ...
    thinkingChars: thinkingBlocks.reduce(
      (s, b) => s + (b.thinking?.length ?? 0),
      0,
    ),
...
// ---- assistant 聚合 CALL 块 ----
function flushAssistantBlock(slot: RequestSlot, msg: EndMessage): void {
...
// ---- user / toolResult 精简块 ----
function flushNonAssistantBlock(msg: EndMessage): void {
